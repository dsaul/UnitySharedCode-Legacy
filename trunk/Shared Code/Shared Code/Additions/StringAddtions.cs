...
namespace OmniLibrary
{
	...
	public static class StringAddtions
	{
		...
		public static string RightAfterLast(string str, string sToFind)
		{
			StringBuilder builder1 = new StringBuilder(str);
			int num1 = str.LastIndexOf(sToFind);
			if (num1 < 0)
		...
		{
			if (!IsNullOrWhiteSpace(str) && str.StartsWith(sStartValue))
			{
		...
		{
			return ReplaceWholeWord(original,wordToFind,replacement, RegexOptions.None);
		}
		...
		/// <returns></returns>
		static public string ReplaceWholeWord(this string original, string wordToFind, string replacement, RegexOptions regexOptions)
		{
		...
		public static string AppendIfNotEmpty(this string str, string prefixBeforeAppend,string valueToAppend)
		{
			return AppendIfNotEmpty(str, prefixBeforeAppend,valueToAppend,"");
		}
		...
		public static string GetStartingNumericFromString(string itmName)
		{
			...
			using (IEnumerator<char> enumerator = itmName.GetEnumerator())
			{
				while (enumerator.MoveNext())
				{
					if (enumerator.Current.CompareTo('0') < 0 || enumerator.Current.CompareTo('9') > 0)
					{
						break;
					}
					safeNumericString += enumerator.Current.ToString();
				}
			}
...
*/
	...
	public static bool Contains(this string source, string toCheck, StringComparison comp)
	{
		...
		{
			#warning TODO: create overloads with exact match  or case sencitive
